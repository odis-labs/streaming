...
let () =
  ...
  T.group "Sink.length" [
    t "no input" ~actual:(into Sink.length Stream.empty) ~expected:0;
    t "finite input" ~actual:(into Sink.length (0--10)) ~expected:10;
  ];
  ...
  T.group "Sink.drain" [
    ...
    t "finite input" ~expected:10 ~actual:begin
      let n = ref 0 in
      Stream.generate 10 (fun i -> incr n; i)
      |> into Sink.drain;
      !n
    end
  ];
  ...
  T.group "Sink.both" [
    t "no input" ~expected:(0, 0) ~actual:(into Sink.(both length sum) Stream.empty);
    t "finite" ~expected:(5, 35) ~actual:(into Sink.(both length sum) (5--10));
  ];
