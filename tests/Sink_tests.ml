...
let () =
  ...
  T.group "Sink.drain"
    [
      t "no input" ~actual:(into Sink.len Stream.empty) ~expected:0;
      t "finite input" ~expected:10
        ~actual:
          begin
            let n = ref 0 in
            Stream.generate ~len:10 (fun i ->
                incr n;
                i)
            |> into Sink.drain;
            !n
  ...
  T.group "Sink.nth"
    [
      t "nth 0, finite input"
        ~actual:(into (Sink.nth 0) (0 -< 10))
        ~expected:(Some 0);
      t "nth 2, no input"
        ~actual:(into (Sink.nth 2) Stream.empty)
        ~expected:None;
      t "nth 2, finite input"
        ~actual:(into (Sink.nth 2) (0 -< 10))
        ~expected:(Some 2);
    ];
  ...
  T.group "Sink.contains"
    [
      t "no input"
        ~actual:(into (Sink.contains ~where:(( = ) 5)) Stream.empty)
        ~expected:false;
      t "must find, finite input"
        ~actual:(into (Sink.contains ~where:(( = ) 5)) (0 -< 10))
        ~expected:true;
      t "must not find, finite input"
        ~actual:(into (Sink.contains ~where:(( = ) 100)) (0 -< 10))
        ~expected:false;
      t "must find, infinite input"
        ~actual:(into (Sink.contains ~where:(( = ) 2)) (Stream.cycle (0 -< 5)))
        ~expected:true;
    ];
  ...
  T.group "Sink.find"
    [
      t "no input"
        ~actual:(into (Sink.find ~where:(( = ) 5)) Stream.empty)
        ~expected:None;
      t "must find, finite input"
        ~actual:(into (Sink.find ~where:(( = ) 5)) (0 -< 10))
        ~expected:(Some 5);
      t "must not find, finite input"
        ~actual:(into (Sink.find ~where:(( = ) 100)) (0 -< 10))
        ~expected:None;
      t "must find, infinite input"
        ~actual:(into (Sink.find ~where:(( = ) 2)) (Stream.cycle (0 -< 5)))
        ~expected:(Some 2);
    ];
    ...
    [
      ...
      t "must find, infinite input"
        ~actual:
          (into
             (Sink.index ~where:(( = ) 'y'))
             (Stream.cycle (Stream.of_list [ 'x'; 'y'; 'z' ])))
        ~expected:(Some 1);
    ];
  ...
  T.group "Sink.minimum"
    [
      t "no input"
        ~actual:(into (Sink.minimum ~by:( < )) Stream.empty)
        ~expected:None;
      t "must find, finite input"
        ~actual:(into (Sink.minimum ~by:( < )) (0 -< 10))
        ~expected:(Some 0);
    ];
  ...
  T.group "Sink.maximum"
    [
      t "no input"
        ~actual:(into (Sink.maximum ~by:( > )) Stream.empty)
        ~expected:None;
      t "must find, finite input"
        ~actual:(into (Sink.maximum ~by:( > )) (0 -< 10))
        ~expected:(Some 9);
    ];
  ...
  T.group "Sink.all"
    [
      t "no input"
        ~actual:(into (Sink.all ~where:(fun x -> x > 5)) Stream.empty)
        ~expected:true;
      t "must be false, finite input"
        ~actual:(into (Sink.all ~where:(fun x -> x > 5)) (0 -< 10))
        ~expected:false;
      t "must be true, finite input"
        ~actual:(into (Sink.all ~where:(fun x -> x > 5)) (6 -< 20))
        ~expected:true;
      t "must be false, infinite input"
        ~actual:
          (into (Sink.all ~where:(fun x -> x < 10)) (Stream.cycle (0 -< 20)))
        ~expected:false;
    ];
  ...
  T.group "Sink.any"
    [
      t "no input"
        ~actual:(into (Sink.any ~where:(fun x -> x > 5)) Stream.empty)
        ~expected:false;
      t "must be false, finite input"
        ~actual:(into (Sink.any ~where:(fun x -> x > 5)) (0 -< 10))
        ~expected:true;
      t "must be true, finite input"
        ~actual:(into (Sink.any ~where:(fun x -> x = 5)) (0 -< 10))
        ~expected:true;
      t "must be true, infinite input"
        ~actual:
          (into (Sink.any ~where:(fun x -> x = 5)) (Stream.cycle (0 -< 10)))
        ~expected:true;
    ];
  ...
  T.group "Sink.list"
    [
      ...
      t "finite input"
        ~actual:(into Sink.list (0 -< 5))
        ~expected:[ 0; 1; 2; 3; 4 ];
      t "infinite input with take"
        ~actual:(into Sink.list Stream.(take 3 (repeat 0)))
        ~expected:[ 0; 0; 0 ];
    ];
  ...
  T.group "Sink.buffer"
    [
      t "empty buffer, no input"
        ~actual:(into (Sink.buffer 0) Stream.empty)
        ~expected:[||];
      T.raises "must fail on negative size"
        ...
        (fun () -> Sink.buffer (-1));
      t "size 4, no input"
        ~actual:(into (Sink.buffer 4) Stream.empty)
        ~expected:[||];
      t "size 4, input 3"
        ~actual:(into (Sink.buffer 4) (0 -< 3))
        ~expected:[| 0; 1; 2 |];
      t "size 4, input 5"
        ~actual:(into (Sink.buffer 4) (0 -< 5))
        ~expected:[| 0; 1; 2; 3 |];
      t "size 2, input 20"
        ~actual:(into (Sink.buffer 2) (0 -< 20))
        ~expected:[| 0; 1 |];
    ];
  ...
  T.group "Sink.product"
    [
      ...
      t "finited input"
        ~actual:(into Sink.product (Stream.of_list [ 1; 4; 2 ]))
        ~expected:8;
      t "finite input with zero"
        ~actual:(into Sink.product (Stream.of_list [ 1; 0; 2 ]))
        ~expected:0;
    ];
  ...
  T.group "Sink.string"
    [
      ...
      t "finited input"
        ~actual:(into Sink.string (Stream.of_list [ "a"; "b"; "c" ]))
        ~expected:"abc";
    ];
  ...
  T.group "Sink.prefilter"
    [
      ...
      t "filter none" ~expected:0
        ~actual:
          (into
             Sink.(prefilter (Fun.const false) sum)
             (Stream.of_list [ 1; 3; 2 ]));
      t "filter all" ~expected:6
        ~actual:
          (into
             Sink.(prefilter (Fun.const true) sum)
             (Stream.of_list [ 1; 3; 2 ]));
      t "filter even" ~expected:2
        ~actual:
          (into
             Sink.(prefilter (fun x -> x mod 2 = 0) sum)
             (Stream.of_list [ 1; 3; 2 ]));
    ];
  ...
  T.group "Sink.prefilter_map"
    [
      ...
      t "filter none" ~expected:0
        ~actual:
          (into
             Sink.(prefilter_map (Fun.const None) sum)
             (Stream.of_list [ 1; 3; 2 ]));
      t "filter all" ~expected:6
        ~actual:
          (into
             Sink.(prefilter_map (fun x -> Some x) sum)
             (Stream.of_list [ 1; 3; 2 ]));
      t "filter even and multiply by 5" ~expected:10
        ~actual:
          (into
             Sink.(
               prefilter_map
                 (fun x -> if x mod 2 = 0 then Some (x * 5) else None)
                 sum)
             (Stream.of_list [ 1; 3; 2 ]));
    ];
  ...
  T.group "Sink.zip"
    [
      t "no input" ~expected:(0, 0)
        ~actual:(into Sink.(zip len sum) Stream.empty);
  ...
  T.group "Sink.distribute"
    [
      t "no input" ~expected:([], [])
        ~actual:(into Sink.(distribute list list) Stream.empty);
      t "finite"
  ...
  T.group "Sink.unzip"
    [
      t "no input" ~expected:([], [])
        ~actual:(into Sink.(unzip list list) Stream.empty);
      t "finite"
        ~expected:([ 1; 2; 3 ], [ 10; 20; 30 ])
        ~actual:
          (into
             Sink.(unzip list list)
             (Stream.of_list [ (1, 10); (2, 20); (3, 30) ]));
    ];
  ...
  T.group "Sink.mean"
    [
      ...
      t "finite small" ~expected:2.0
        ~actual:(into Sink.mean (Stream.of_list [ 1.0; 2.0; 3.0 ]));
      t "finite zeros" ~expected:0.0
        ~actual:(into Sink.mean (Stream.of_list [ 0.0; 0.0; 0.0 ]));
    ]
