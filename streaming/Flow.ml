...
type ('a, 'b) t = ('a, 'b) flow = { flow : 'r. ('b, 'r) sink -> ('a, 'r) sink }
[@@unboxed]
...
let take_while pred =
  let flow (Sink k) =
    ...
    let push (acc, _) x =
      if pred x then (k.push acc x, true) else (acc, false)
    in
...
let drop n =
  let flow (Sink k) =
    let init () = (k.init (), 0) in
    let push (acc, i) x = if i < n then (acc, i + 1) else (k.push acc x, i) in
...
let drop_while p =
  let flow (Sink k) =
    ...
    let push (acc, dropping) x =
      if p x && dropping then (acc, true) else (k.push acc x, false)
    in
    let full (acc, _) = k.full acc in
    let stop (acc, _) = k.stop acc in
    Sink { init; push; full; stop }
