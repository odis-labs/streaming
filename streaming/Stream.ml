...
type 'a t = 'a stream = { stream : 'r. ('a, 'r) sink -> 'r } [@@unboxed]
...
let triple x1 x2 x3 =
  let stream (Sink k) =
    k.stop (k.push (k.push (k.push (k.init ()) x1) x2) x3)
  in
  { stream }
...
let count n =
  let stream (Sink k) =
    let rec loop s r = if k.full r then r else loop (s + 1) (k.push r s) in
...
let[@inline] range ?by:(step = 1) n m =
  if n > m then invalid_arg "Streaming.Stream.range: invalid range"
  else
    let[@inline] stream (Sink k) =
      let rec loop i r =
        if k.full r then r
        else if i >= m then r
        else loop (i + step) (k.push r i)
      in
      k.stop (loop n (k.init ()))
    in
    { stream }
...
let[@inline] cartesian_range ?by_i:(step_i = 1) ?by_j:(step_j = 1) f n0 m0 n1 m1
  ...
  if n0 > m0 || n1 > m1 then
    ...
    let[@inline] stream (Sink k) =
      let rec loop i j r =        
        if k.full r then r
        else if i >= m0 && j < m1 then loop n0 (j + step_j) r
        else if i >= m0 then r
        else loop (i + step_i) j (k.push r (f i j))
      in
...
let repeat ?times:n x =
  let stream (Sink k) =
    match n with
    | None ->
      let rec loop r = if k.full r then r else loop (k.push r x) in
      bracket loop ~init:k.init ~stop:k.stop
    | Some n ->
      let rec loop i r =
        if k.full r || i = n then r else loop (i + 1) (k.push r x)
      in
...
let repeatedly ?times:n f =
  let stream (Sink k) =
    match n with
    | None ->
      let rec loop r = if k.full r then r else loop (k.push r (f ())) in
      bracket loop ~init:k.init ~stop:k.stop
    | Some n ->
      let rec loop i r =
        if k.full r || i = n then r else loop (i + 1) (k.push r (f ()))
      in
      bracket (loop 0) ~init:k.init ~stop:k.stop
  in
  { stream }
...
let flat_map f this =
  let stream (Sink k) =
    let push r x =
      (f x).stream (Sink { k with init = (fun () -> r); stop = (fun r -> r) })
    in
...
let concat this that =
  let stream (Sink k) =
    let stop r =
      if k.full r then k.stop r
      else that.stream (Sink { k with init = (fun () -> r) })
    in
...
let split ~by:pred self =
  let stream (Sink k) =
    ...
    let push (r, acc) x =
      if pred x then (k.push r acc, empty) else (r, acc ++ single x)
    in
